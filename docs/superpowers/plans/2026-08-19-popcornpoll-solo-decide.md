...
- `POST /api/solo/pick` checks `req.headers.get('origin') === config.appOrigin` before writing to `match_history`, matching `server/http/rooms.ts`'s existing origin check for the one other state-mutating room-creation endpoint. `GET /api/solo/pool` and `POST /api/solo/surprise` skip the origin check. `surprise` really is read-only. `pool` is not, strictly speaking — `buildPool` calls `stampLastUsed()` on every returned row and, under `candidateSource=plex+tmdb`, calls `tmdb.discoverMovies()` (outbound third-party network) plus `upsertTmdbOnlyRow()`/`mergeTmdbOnlyIntoPlexRow()` (new/merged rows in `movies`) — so `pool` is a mutating, third-party-calling GET reachable cross-origin from any page a user visits. It stays without an `Origin` check anyway because browsers don't send `Origin` on same-origin GETs, so adding that check would reject legitimate same-origin traffic, not just cross-origin — `Origin` is a POST-body-carrying-request mechanism, not a general CSRF defense for GETs. The actual mitigations here are: the response is unreadable cross-origin (no CORS headers sent), the writes are non-attacker-controlled TMDB discover data (not user input), the endpoint is rate-limited (see below), and the app's documented posture is trusted-LAN, not public-internet. `Sec-Fetch-Site: same-origin` would be the header that actually works on a GET, if tighter defense is ever wanted. Only `GET /api/solo/pool` gets a rate-limit bucket (mirrors `server/http/rooms.ts`'s bucket) — it is the only solo endpoint that can trigger a TMDB network call via `buildPool`; `surprise` and `pick` only touch this app's own SQLite and are cheap.
